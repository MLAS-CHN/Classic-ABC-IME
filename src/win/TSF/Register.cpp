...
#define CHECK_HR(hr) \
  ...
  // IME not support these languages, so HKL is NULL
  ...
  if (fRet = RegCreateKeyExA(HKEY_CLASSES_ROOT, achIMEKey, 0, NULL,
    ...
    if (fRet &=
...
#ifdef _M_ARM64
      {
        ...
        // When loaded, abcime.dll will be redirected to abcimeARM64.dll on
        // ARM64 processes, and abcimex64.dll on x64 processes.
        //
        // But GetModuleFileNameA will return the actual loaded DLL name aka
        // abcimeARM64.dll Rewrite the path to point to the redirector.

        char wrapperPath[MAX_PATH];
        StringCbCatA(achFileName, MAX_PATH, "\\..\\abcime.dll");
