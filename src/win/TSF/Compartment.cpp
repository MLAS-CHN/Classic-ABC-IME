...
HRESULT TSF::_SetKeyboardOpen(BOOL fOpen) {
  HRESULT hr = E_FAIL;
  com_ptr<ITfCompartmentMgr> pCompMgr;
...
BOOL TSF::_InitCompartment() {
  using namespace std::placeholders;

  auto callback = std::bind(&TSF::_HandleCompartment, this, _1);
  _pKeyboardCompartmentSink = new CCompartmentEventSink(callback);
  if (!_pKeyboardCompartmentSink)
    return FALSE;
  return SUCCEEDED(_pKeyboardCompartmentSink->_Advise(
      (IUnknown*)_pThreadMgr, GUID_COMPARTMENT_KEYBOARD_OPENCLOSE));
}

void TSF::_UninitCompartment() {
  if (_pKeyboardCompartmentSink) {
